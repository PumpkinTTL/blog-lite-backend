...
@Unique('uq_entity_visibility', ['entityType', 'entityId', 'subjectType', 'subjectId'])
...
export class EntityVisibilityEntity {
  ...
  @Column({ type: 'varchar', length: 50, name: 'entity_type', comment: '实体类型：post/announcement/...' })
  entityType: string;

  @Column({ type: 'int', name: 'entity_id', comment: '实体 ID' })
  entityId: number;

  @Column({ type: 'varchar', length: 10, name: 'subject_type', comment: '主体类型：user/role' })
  subjectType: VisibilitySubjectType;

  @Column({ type: 'int', name: 'subject_id', comment: '主体 ID（用户 ID 或角色 ID）' })
  subjectId: number;
